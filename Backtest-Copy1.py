...
class Management:
    ...
    def get_day_return(self,day,rolling_time,shift_time,signal_time,std_in,std_out,times=3):
        ...
        return whole_profit,trans_all_info
    ...
    def get_one_time_profit(self,times,signal_df,ratio_df,pos_X,pos_Y):
        ...
        return temp_profit,transaction_all
...
tackle_args_dict = {"rolling_time":"10min",                    "shift_time":"1min",                    "signal_time":"1min",                    "std_in":1,                    "std_out":2.5,                   "initial_money":2000*int(1e4),                   "times":15}

date_range = sorted(list(os.walk("/Users/dt/Desktop/HA/ETF_rolling/data/510500"))[0][1])
Mana = Management(date_range)
ret,transaction_info = Mana.run(**tackle_args_dict)


# In[227]:


pd.Series(ret).sort_values(ascending=False)


# In[228]:


def transfer_res(d_dict):
    res = {}
    for day in d_dict.keys():
        temp = d_dict[day]
        temp_dict = {}
        temp_dict_value = list(temp.values())
        for value in temp_dict_value:
            temp_dict.update(value)
        res[day] = temp_dict
    return res
        
transfer_transaction_info = transfer_res(transaction_info["20210610"])


# In[229]:


pd.DataFrame(transfer_transaction_info).T


# In[205]:


signal_res = signal2_action_change.dropna().resample("3s").apply(lambda x:test(x)).dropna()
...
signal_res = signal2_action_change.dropna().resample("15s").apply(lambda x:test(x)).dropna()

temp = signal_res.dropna()
t2 = temp[(temp["action"] == temp["shift"]) & ((temp["action"]!="None")&(temp["shift"]!="None"))]
...
def get_first(df):
    res = pd.DataFrame([])
    raw = df.iloc[0,0]
    res = pd.concat([res,df.iloc[[0]]],axis=0)
    for i in range(1,df.shape[0]):
        if df.iloc[i,0] != raw:
            res = pd.concat([res,df.iloc[[i]]],axis=0)
            raw = df.iloc[i,0]
            ...
            continue
    return res
get_first(t1).to_csv("/Users/dt/Desktop/decision.csv")


# In[223]:


ratio_df.to_csv("/Users/dt/Desktop/ratio.csv")
